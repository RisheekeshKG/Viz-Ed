...
from main import checking, checking_outside, generate_frames,pts,fun_imgG, fun_imgGray, contour
...
def CapturingPage(level):
    ...
    if f"levels_passed.{section}" in user and current_level - 1 in user['levels_passed'].get(section, []):
...
def test(level):
    ...
    # Check if the previous level has been completed
    if f"levels_passed.{section}" in user and 4 in user['levels_passed'].get(section, []):
        # Allow access to the test for this level
